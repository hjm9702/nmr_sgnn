...
if not os.path.isfile(f'result/result.csv'):
    with open(f'result/result.csv', 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(['message_passing_mode', 'readout_mode', 'graph_representation', 'target', 'fold_seed', 'test_mae', 'memo'])

with open(f'result/result.csv', 'a', newline='') as f:
    w = csv.writer(f)
    w.writerow([args.message_passing_mode, args.readout_mode, args.graph_representation, args.target, args.fold_seed, test_mae, args.memo])
