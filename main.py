...
if not os.path.isfile(f'result/result.csv'):
    with open(f'result/result.csv', 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(['message_passing_mode', 'readout_mode', 'graph_representation', 'target', 'seed', 'feat_mode', 'test_mae', 'test_rmse', 'time_per_mol'])

with open(f'result/result.csv', 'a', newline='') as f:
    w = csv.writer(f)
    w.writerow([args.message_passing_mode, args.readout_mode, args.graph_representation, args.target, args.seed, args.feat_mode, test_mae, test_rmse, time_per_mol])
